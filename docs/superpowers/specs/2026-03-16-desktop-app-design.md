...
A new orchestration package that extracts the logic currently living in `apps/cli/src/timingSources.ts` and the command action bodies in `apps/cli/src/index.ts`.

### Exports

Functions only — all types are owned by `@racedash/core`:
...
export async function joinVideos(opts: JoinOptions): Promise<void>
...
| `@racedash/engine` | Timing/config types: `SegmentConfig`, `TimingCapabilities`, `LoadedTimingConfig`, `ResolvedTimingSegment`, `PositionOverrideConfig`, `ManualTimingEntry`, engine function option/result types |
...
`renderSession` accepts an `onProgress` callback instead of writing to stderr. The CLI passes a callback that formats the terminal progress bar. The desktop passes a callback that sends an IPC event to the renderer to update a progress UI. This allows both consumers to drive progress display in their own way.
...
Handles all heavy work: calling `@racedash/engine`, native file dialogs, FFmpeg detection and download, app lifecycle. Registers IPC handlers for each engine operation.
...
React app. Calls `window.racedash.*` and receives progress events back via IPC. No awareness of the filesystem or engine internals.
...
**Download sources:**
- macOS: `evermeet.cx` (widely-used static macOS FFmpeg builds)
- Windows: `gyan.dev` (the recommended Windows FFmpeg build source)
...
`pnpm test` runs on both runners before building — a failing test blocks the release.
...
- **Join** — concatenate GoPro chapter files
