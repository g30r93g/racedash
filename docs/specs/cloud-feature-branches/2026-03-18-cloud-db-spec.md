...
3. **Slot token double-signaling prevention:** `claimNextQueuedSlotToken` uses `FOR UPDATE SKIP LOCKED` to ensure that concurrent Lambda invocations cannot claim the same token. The CTE captures the pre-update `slot_task_token` value before setting it to `NULL`, and `RETURNING target.slot_task_token` returns the original token to the caller. Only the Lambda that successfully claims the row receives the token.
