...
infra/
  ...
  test/
    ...
    localstack/
      s3-upload.localstack.test.ts
      sqs-dispatch.localstack.test.ts
      step-functions.localstack.test.ts
      ses-email.localstack.test.ts
      eventbridge-relay.localstack.test.ts
  localstack-init/
    setup.sh                         # Bootstrap script — creates local resources
    state-machine.asl.json           # Local ASL definition (generated, see scripts/)
  scripts/
    generate-local-asl.ts            # Patches CDK-synthesized ASL for LocalStack
  docker-compose.localstack.yml
  .env.localstack                    # Generated by setup.sh (git-ignored)
  cdk.json
  tsconfig.json
  package.json
  jest.config.ts
  jest.localstack.config.ts          # Separate Jest config for LocalStack tests
```
...
LocalStack provides local emulation of AWS services for development and integration testing without incurring AWS costs or requiring a network connection. The `cloud-infra` branch includes a LocalStack configuration for testing the core pipeline locally.

### Supported Services

| Service | LocalStack Support | Usage in RaceDash |
|---|---|---|
| S3 | Full | Upload/render buckets, multipart upload, lifecycle rules, presigned URLs |
| SQS | Full | Social upload queue + DLQ |
| Step Functions | Full (Standard Workflows) | Render pipeline state machine execution |
| Lambda | Full | Pipeline handler invocation, dispatch Lambda |
| SES | Partial (email capture, no delivery) | Render completion/failure emails (captured, not sent) |
| EventBridge | Full | Terminal state relay rule |
| IAM | Partial (policies stored, not enforced) | Role creation only — IAM enforcement is not tested locally |
| CloudFront | Not supported | Signed URL generation tested via unit tests against the `@aws-sdk/cloudfront-signer` library directly |
| MediaConvert | Not supported | `PrepareComposite` Lambda tested in isolation (returns job config); `RunMediaConvert` state mocked in local state machine definition |
| ECS Fargate | Not supported | YouTube upload Fargate task tested independently; dispatch Lambda's `RunTask` call mocked |

### Setup

**Docker Compose** (`infra/docker-compose.localstack.yml`):
...
**Bootstrap script** (`infra/localstack-init/setup.sh`):

Creates the local resources that mirror CDK stack outputs:
1. S3 buckets: `racedash-uploads-local`, `racedash-renders-local`
2. SQS queue: `racedash-social-upload-local` + DLQ `racedash-social-upload-dlq-local`
3. SES email identity (verified automatically in LocalStack)
4. Step Functions state machine (uploaded from a local ASL definition)

The bootstrap script outputs a `.env.localstack` file with endpoint URLs and resource ARNs for use by integration tests and local development.

**Package scripts** (`infra/package.json`):

```json
{
  "scripts": {
    "localstack:up": "docker compose -f docker-compose.localstack.yml up -d",
    ...
    "test:local": "pnpm localstack:up && dotenv -e .env.localstack -- jest --config jest.localstack.config.ts && pnpm localstack:down",
    "test:local:watch": "dotenv -e .env.localstack -- jest --config jest.localstack.config.ts --watch"
  }
}
```
...
All Lambda handlers and API code must accept an optional `endpoint` override for AWS SDK clients. This is implemented via a shared client factory:

```ts
// infra/lambdas/shared/aws-clients.ts
import { S3Client } from '@aws-sdk/client-s3'
import { SFNClient } from '@aws-sdk/client-sfn'
import { SQSClient } from '@aws-sdk/client-sqs'
import { SESClient } from '@aws-sdk/client-ses'

const endpoint = process.env.AWS_ENDPOINT_URL  // set to http://localhost:4566 in local dev
...
When `AWS_ENDPOINT_URL` is set (local dev), all SDK calls route to LocalStack. When unset (production), standard AWS endpoint resolution applies.

### LocalStack Integration Tests

Location: `infra/test/localstack/`

These tests start the full local pipeline and verify end-to-end behavior against emulated AWS services.

**`infra/test/localstack/s3-upload.localstack.test.ts`:**

1. Multipart upload to `racedash-uploads-local` succeeds and object is retrievable.
2. Presigned PUT URL allows upload from an HTTP client.
3. Presigned GET URL allows download.
4. Lifecycle rules are configured (verifiable via `GetBucketLifecycleConfiguration`).

**`infra/test/localstack/sqs-dispatch.localstack.test.ts`:**

1. Sending a message to the social upload queue makes it visible to a consumer.
2. Message with invalid format ends up in the DLQ after max receives.
3. Visibility timeout is 2700 seconds.

**`infra/test/localstack/step-functions.localstack.test.ts`:**

1. State machine can be started with a valid input payload.
2. `WaitForSlot` callback state pauses execution until `SendTaskSuccess` is called.
3. Calling `SendTaskSuccess` on the `WaitForSlot` task token advances to `GrantSlot`.
4. State machine reaches `Succeed` terminal state when all callbacks are resolved.
5. State machine routes to `ReleaseCreditsAndFail` when a callback state times out (use accelerated heartbeat for testing).
6. State machine `RunMediaConvert` state is mocked (LocalStack doesn't support MediaConvert) — replaced with a Pass state in the local ASL definition that returns a mock MediaConvert response.

**`infra/test/localstack/ses-email.localstack.test.ts`:**

1. `SendEmail` call succeeds against LocalStack SES.
2. Sent emails are captured and retrievable via LocalStack's `_ses/` internal endpoint for assertion.

**`infra/test/localstack/eventbridge-relay.localstack.test.ts`:**

1. Publishing a Step Functions terminal state event to EventBridge triggers the relay Lambda.
2. Relay Lambda receives the correct event payload.

### Local State Machine Definition

The production state machine uses CDK L2 constructs (synthesized to ASL). For LocalStack, a local ASL definition is maintained at `infra/localstack-init/state-machine.asl.json` with these modifications from production:

1. `RunMediaConvert` — replaced with a `Pass` state that outputs a mock MediaConvert job response (LocalStack doesn't support `mediaconvert:createJob.sync`).
2. Lambda ARNs — use `arn:aws:lambda:us-east-1:000000000000:function:{name}` format (LocalStack's default account).
3. All other states (WaitForSlot, GrantSlot, StartRenderOverlay, PrepareComposite, FinaliseJob, NotifyUser, ReleaseCreditsAndFail) — identical behavior to production.

> **Note:** The local ASL definition is generated from the CDK-synthesized production ASL via a script (`infra/scripts/generate-local-asl.ts`) that patches the MediaConvert state and rewrites Lambda ARNs. This prevents the local definition from drifting out of sync with the CDK source of truth.

### Limitations

1. **IAM is not enforced** — LocalStack stores IAM policies but does not evaluate them. IAM correctness is verified by CDK assertion tests (specification + property-based tests), not by LocalStack.
2. **CloudFront is not emulated** — Signed URL generation is tested via unit tests against the `@aws-sdk/cloudfront-signer` library. The CloudFront distribution itself is only testable after real AWS deployment.
3. **MediaConvert is not emulated** — The `PrepareComposite` Lambda is tested in isolation (it produces a MediaConvert job config JSON). The actual `createJob.sync` integration is only testable after deployment. The local state machine mocks this state with a Pass.
4. **ECS Fargate is not emulated** — The SQS dispatch Lambda's `RunTask` call is mocked in local tests. The YouTube Fargate task handler has its own unit tests independent of ECS.
5. **Lambda execution model** — LocalStack runs Lambdas in Docker containers. Cold start behavior differs from real Lambda. Performance characteristics are not representative.

### Environment Variables for Local Development

The `.env.localstack` file generated by the bootstrap script:
...
All tests live in `infra/test/` and use Jest with `aws-cdk-lib/assertions`. LocalStack integration tests live in `infra/test/localstack/` and require Docker.
