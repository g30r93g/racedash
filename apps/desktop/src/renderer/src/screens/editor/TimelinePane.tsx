...
export function TimelinePane({ project, videoInfo }: TimelinePaneProps): React.ReactElement {
  ...
  /** Convert seconds to a CSS left-percentage in the track area. */
  const pct = (seconds: number) => `${Math.min(100, (seconds / duration) * 100).toFixed(3)}%`
  /** Convert a duration in seconds to a CSS width-percentage. */
  const widthPct = (seconds: number) => `${Math.min(100, (seconds / duration) * 100).toFixed(3)}%`

  /** Compute segment start/end in seconds from videoOffsetFrame. */
  ...
  // Placeholder laps — replaced once ProjectData includes real lap timing.
    ...
    <div className="flex h-[180px] shrink-0 flex-col border-t border-border bg-background" style={{ fontSize: 11 }}>
      ...
      <div className="flex h-8 shrink-0 items-center justify-between border-b border-border px-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button aria-label="Zoom out" className="flex h-5 w-5 items-center justify-center rounded border border-border text-muted-foreground hover:border-primary hover:text-foreground">−</button>
          <button aria-label="Zoom in"  className="flex h-5 w-5 items-center justify-center rounded border border-border text-muted-foreground hover:border-primary hover:text-foreground">+</button>
        </div>
      </div>
