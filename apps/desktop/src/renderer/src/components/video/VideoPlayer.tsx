...
export const VideoPlayer = React.forwardRef<HTMLVideoElement, VideoPlayerProps>(function VideoPlayer(
  ...
  const overlayLayout = React.useMemo(() => {
    ...
    const videoScale = Math.min(containerSize.width / referenceVideoWidth, containerSize.height / referenceVideoHeight)

    const displayedVideoWidth = referenceVideoWidth * videoScale
    const displayedVideoHeight = referenceVideoHeight * videoScale
    const effectiveCompositionWidth = entry.scaleWithVideo ? referenceVideoWidth : entry.width
