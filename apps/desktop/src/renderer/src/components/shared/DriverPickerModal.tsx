...
export function DriverPickerModal({
  ...
  const drivers =
    driversResult?.segments.flatMap((seg) => seg.drivers.map((d) => ({ kart: d.kart, name: d.name }))) ?? []
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[400px] max-h-[70vh] flex flex-col">
        ...
        {loading && <p className="text-xs text-muted-foreground py-4">Loading drivers…</p>}
                ...
                <TableBody>
                  {drivers.map((d) => (
                    ...
                    >
                      ...
                      <TableCell className="py-1.5 text-sm text-foreground font-medium">{d.name}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
