...
function buildBgPath(w: number, h: number): string {
  ...
  return [
    ...
    `l ${x(-4)},${y(-6)}`,                                                                          // right notch diagonal
    ...
    `H ${x(31)}`,                                                                                   // center flat (rises to top)
...
export const BannerBackground: React.FC<BannerBackgroundProps> = ({
  ...
  <svg width={width} height={height} style={{ position: 'absolute', inset: 0 }}>
    ...
    <path d={buildTimerPath(width, height)} fill={timerFill} opacity={opacity} />
  </svg>
)
